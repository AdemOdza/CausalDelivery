...
public class Dispatcher extends Thread implements Runnable {
    ...
    public void run() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            // Increment clock once per broadcast
            ...
            // Build message and send to all processes
            ...
            synchronized (Main.socketMap) {
                for (Short process : Main.socketMap.keySet()) {
                    ...
                    Message msg;
                    synchronized (Main.vectorClock) {
                        msg = new Message(self, process, Command.MESSAGE, body, Main.vectorClock);
                    }
                    try {
        ...
        synchronized (Main.socketMap) {
            ...
            // Increment clock before sending termination messages
            synchronized (Main.clock) {
                synchronized (Main.vectorClock) {
                    Main.clock.increment();
                    Main.vectorClock.put(self, Main.clock.getTime());
            ...
            for (Short process : Main.socketMap.keySet()) {
                ...
                Main.logger.out(String.format("Dispatcher: Sending termination to %s", Main.generateUrl(process)));
                Message msg = new Message(self, process, Command.TERMINATE, "", Main.vectorClock);
                try {
                    writer = new PrintWriter(new OutputStreamWriter(Main.socketMap.get(process).getOutputStream(), StandardCharsets.UTF_8), true);
                } catch (IOException e) {
                    throw new RuntimeException(e);
    ...
    public Socket makeConnection(Short process) throws IOException, InstantiationException {
        ...
        Message msg;
        synchronized (Main.vectorClock) {
            msg = new Message(self, process, Command.INIT, Main.vectorClock);
        }
        writer.append(msg.toString()).append('\n');
        writer.flush();
